...
for i in $(seq 1 $MAX_ITERATIONS); do
  ...
  if [[ "$TOOL" == "amp" ]]; then
    ...
    # Codex: run non-interactively from the project root while keeping Ralph state in SCRIPT_DIR.
    OUTPUT=$({
      echo "Ralph runtime paths:"
    ...
    } | codex exec --full-auto -C "$PROJECT_ROOT" - 2>&1 | tee /dev/stderr) || true
  fi
